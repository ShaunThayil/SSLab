...
def headMaker(source,objectcode):
    ...
    l=normalize(hex(int(length)),6,'0')
...
def sic_2pass(source):
    #Loading Data
    objectcode=open("newobjcode.txt",'w')
    
    loc=headMaker(source,objectcode)
    textrec=0
    line=createText(loc)
    recsize=0
    k=0
    for data in source.readlines():
        res,inc=0,0
        code=data.split('#')
        if(code[0]==''):
            continue
        ...
        if(recsize>30 or res==1):
                    #reset record size variable 
            print("Record FULL!!")
            print(textrec," ",res)
            line.append('\n')
            
            line.insert(2,normalize(hex(textrec),2,'0'))
            print(line)
            recsize=textrec=0
            objectcode.write("^".join(line)+'\n')
            k=0
        ...
        if(textrec==0  and k==0):      #Create New  Record  
            k=1
            print("Inside Creating New Text Record"+location)
            line=createText(location)
            print(line)

        textrec=textrec+inc
        print("Textrec Updating:",textrec," ",recsize)
        line.append(command)
        
        
    objectcode.write("E^"+normalize(symtab[operand],6,'0'))
    objectcode.close()
